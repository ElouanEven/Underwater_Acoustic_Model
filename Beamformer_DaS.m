function [ENERGY, list_angle_detected, list_time_detected] = Beamformer_DaS(SIG_noisy, REC_z, Lfilt, F, PHI, c_iso, T_0, T_snap, threshold)
    % Beamformer_DaS - Applying the Delay and Sum Beamformer to find the direction of arrival over the input signal
    %
    % Inputs:
    %    SIG_noisy - Signal arriving on each sensor
    ...
    for t_snap = 1:length(T_snap)
        ...
        [maxValue, indexOfMax] = max(mean(Energy,1));
        if maxValue > threshold
            list_angle_detected = [list_angle_detected, PHI(indexOfMax)-90]; % To compare with theta of arrival on the hydrophone array
            list_time_detected = [list_time_detected, T_0(T_snap(t_snap))];
        end
    end
end